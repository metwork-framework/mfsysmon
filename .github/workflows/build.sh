...
#if test -f "${BUILDCACHE}/build_hash_mfsysmon_${BRANCH}_`cat .build_hash`"; then
...
rm -Rf html_doc
if test -d /opt/metwork-mfsysmon-${TARGET_DIR}/html_doc; then cp -Rf /opt/metwork-mfsysmon-${TARGET_DIR}/html_doc . ; fi
...
mkdir rpms
mv /opt/metwork-mfsysmon-${TARGET_DIR}/*.rpm rpms

#rm -f ${BUILDCACHE}/build_hash_mfsysmon_${BRANCH}_*
#touch ${BUILDCACHE}/build_hash_mfsysmon_${BRANCH}_`cat .build_hash`
#ls -l ${BUILDCACHE}
